...
def _get_positive_src_file_list(
    lines,
    solution,
    smell_type,
        ...
        if smell_type == "Design":
            file = os.path.join(
                os.path.join(
                    os.path.join(
                        os.path.join(
                            os.path.join(code_split_out_folder_class, solution),
                            solution,
                        ),
            ...
            file = os.path.join(
                os.path.join(
                    os.path.join(
                        os.path.join(
                            os.path.join(code_split_out_folder_method, solution),
                            solution,
                        ),
                        str(_filter_str(tokens[1])),
                    ),
...
def _put_files_in_right_bucket(
    pos_source_file_list,
    solution,
    positive_cases_folder,
        ...
        for file in files:
            ...
            if smell_type == "Design":
                namespace = root.replace(
                    code_split_out_folder_class + os.path.sep, ""
                ).replace(os.path.sep, "_")
            else:
                namespace = root.replace(
                    code_split_out_folder_method + os.path.sep, ""
                ).replace(os.path.sep, "_")

            if _is_present(pos_source_file_list, src_file_path):
                dest_file_path = os.path.join(
                    positive_cases_folder, namespace + str(pos_counter) + file
                )
                if not os.path.exists(dest_file_path):
                    ...
                    print(
                        "File already exists: " + str(dest_file_path)
                    )  # This should not be the case
            else:
                dest_file_path = os.path.join(
                    negative_cases_folder, namespace + str(neg_couter) + file
                )
                if not os.path.exists(dest_file_path):
                    try:
                        shutil.copyfile(src_file_path, dest_file_path)
...
def _scan_solution(
    ...
    for root, dirs, files in os.walk(os.path.join(smells_results_folder, solution)):
        for file in files:
            ...
            pos_source_files = _get_positive_src_file_list(
                lines,
                solution,
                smell_type,
                code_split_out_folder_class,
    ...
    total_copied_files = _put_files_in_right_bucket(
        pos_source_file_list,
        solution,
        positive_cases_folder,
        negative_cases_folder,
...
def generate_data(
    ...
    for smell in range(len(SMELL_NAME_LIST)):
        ...
        positive_cases_folder = os.path.join(
            learning_data_folder_path, SMELL_NAME_LIST[smell], "Positive"
        )
        negative_cases_folder = os.path.join(
            learning_data_folder_path, SMELL_NAME_LIST[smell], "Negative"
        )
        ...
        for solution in os.listdir(smells_results_folder):
            _scan_solution(
                ...
                positive_cases_folder,
                negative_cases_folder,
                SMELL_NAME_STR_LIST[smell],
                SMELL_TYPE_LIST[smell],
                smells_results_folder,
                code_split_out_folder_class,
                code_split_out_folder_method,
            )
