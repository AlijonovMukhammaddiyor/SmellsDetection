...
def _analyze_projects(
    batch_file_path, repo_name, result_folder_base, designite_console_path
):
...
def analyze_repositories(
    repo_source_folder, batch_files_folder, result_folder_base, designite_console_path
):
    if not os.path.exists(result_folder_base):
        os.makedirs(result_folder_base)
    for folder in os.listdir(repo_source_folder):
        project_paths = _get_all_csharp_projects_paths(
            os.path.join(repo_source_folder, folder)
        )
        if len(project_paths) > 0:
            print("Processing " + folder)
            batch_file_path = _write_batch_file(
                folder, project_paths, batch_files_folder
            )
            _analyze_projects(
                batch_file_path, folder, result_folder_base, designite_console_path
            )
