...
def _run_code_split(
    ...
    if sln_file is not None:
        ...
        if not os.path.exists(out_folder):
            os.mkdir(out_folder)
            subprocess.call(
                [code_split_exe_path, sln_file, code_split_mode, out_folder]
            )


def cs_code_split(
    repo_base_folder, code_split_result_folder, code_split_mode, code_split_exe_path
):
    ...
    for dir in os.listdir(repo_base_folder):
        ...
        if os.path.exists(os.path.join(code_split_result_folder, dir)):
            ...
            _run_code_split(
                dir,
                os.path.join(repo_base_folder, dir),
                code_split_result_folder,
                code_split_exe_path,
                code_split_mode,
            )
