...
def _is_contain_overloaded_methods(input_file, tokenizer_path, tokenizer_language):
    ...
    with open(temp_file, "w", errors="ignore") as tok_out_file:
        process = subprocess.Popen(
            [tokenizer_path, "-l", tokenizer_language, "-o", "method", input_file],
            bufsize=10240000,
            stdout=tok_out_file,
        )
        process.wait()
...
def _run_tokenizer(
    folder_path, out_folder, tokenizer_path, tokenizer_language, tokenizer_level
):
    ...
    for file in os.listdir(folder_path):
        ...
        with open(in_file, "w", errors="ignore") as tok_out_file:
            ...
            subprocess.run(
                [exe_path, "-l", tokenizer_language, "-o", tokenizer_level, input_file],
                stdout=tok_out_file,
            )
            # process.wait()
        ...
        if os.path.getsize(out_file) > 52428800:  # 50 mb
            ...
            out_file = os.path.join(
                out_folder, "tokenized" + str(file_counter) + ".tok"
            )
...
def tokenize(
    tokenizer_language,
    tokenizer_input_base_path,
    tokenizer_out_base_path,
    tokenizer_exe_path,
):
    ...
    for dim in {2}:
        for dir in list:
            ...
            out_folder = os.path.join(
                os.path.join(os.path.join(tokenizer_out_base_path, dir), dim_str),
                "Positive",
            )
            _run_tokenizer(
                cur_folder,
