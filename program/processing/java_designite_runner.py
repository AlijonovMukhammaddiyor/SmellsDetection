...
def _run_designite_java(
    folder_name, folder_path, designiteJava_jar_path, smells_results_folder
):
    ...
    proc = Popen(
        ["java", "-jar", designiteJava_jar_path, "-i", folder_path, "-o", out_folder]
    )
    proc.wait()
...
def _build_project(dir, dir_path):
    ...
    if os.path.exists(pom_path):
        ...
        proc = Popen(
            [
                r"C:\Program Files (x86)\apache-maven-3.6.2\bin\mvn.cmd",
                "clean",
                "install",
                "-DskipTests",
            ]
    ...
    if os.path.exists(gradle_path):
        ...
        os.chdir(dir_path)
        proc = Popen(
            [r"C:\Program Files\Gradle\gradle-5.6.2\bin\gradle.bat", "compileJava"]
        )
        proc.wait()
        is_compiled = True
    if not is_compiled:
...
def analyze_repositories(
    repo_source_folder, smells_results_folder, designiteJava_jar_path
):
    for dir in os.listdir(repo_source_folder):
        ...
        if os.path.exists(os.path.join(smells_results_folder, dir)):
            ...
            _run_designite_java(
                dir,
                os.path.join(repo_source_folder, dir),
