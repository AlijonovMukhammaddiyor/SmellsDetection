...
TRAINING_TOKENIZER_OUT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "data",
    "tokenizer_cs",
)

RQ1_OUT_FOLDER = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "results",
    "rq1",
    "raw",
)
RQ2_OUT_FOLDER = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "results",
    "rq2",
    "raw",
)
EVAL_TOKENIZER_OUT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "data",
    "tokenizer_java",
)
...
def embedding_lstm(
    ...
    config,
    smell,
    rq1_out_folder=RQ1_OUT_FOLDER,
    rq2_out_folder=RQ2_OUT_FOLDER,
    dim=DIM,
):
    ...
    model.add(
        tf.keras.layers.Embedding(
            input_dim=max_features + 1, output_dim=config.emb_output, mask_zero=True
        )
    )
    ...
    model.add(
        tf.keras.layers.LSTM(config.lstm_units, recurrent_dropout=0.1, dropout=0.1)
    )
    model.add(tf.keras.layers.Dropout(config.dropout))
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
        filepath=best_model_filepath, monitor="val_loss", verbose=1, save_best_only=True
    )
    ...
    rq1_prob = model.predict(rq1_data.eval_data)
    rq1_y_pred = inputs.get_predicted_y(rq1_prob, CLASSIFIER_THRESHOLD)

    rq2_prob = model.predict(rq2_data.eval_data)
    ...
    (
        ...
        rq1_recall,
        rq1_f1,
        rq1_average_precision,
        rq1_fpr,
        rq1_tpr,
    ) = metrics_util.get_all_metrics(
        model, rq1_data.eval_data, rq1_data.eval_labels, rq1_y_pred
    )
    (
        rq2_auc,
        rq2_accuracy,
        ...
        rq2_recall,
        rq2_f1,
        rq2_average_precision,
        rq2_fpr,
        rq2_tpr,
    ) = metrics_util.get_all_metrics(
        model, rq2_data.eval_data, rq2_data.eval_labels, rq2_y_pred
    )

    # plot_util.save_roc_curve(fpr, tpr, auc, smell, config, out_folder, DIM)
    plot_util.save_precision_recall_curve(
        rq1_data.eval_labels,
        rq1_y_pred,
        rq1_average_precision,
        smell,
        config,
...
def get_all_data(data_path, rq2_eval_data_path, smell):
    ...
    (
        ...
        data_path, train_validate_ratio=TRAIN_VALIDATE_RATIO, max_training_samples=5000
    )
    ...
    rq1_data = input_data.Input_data(
    ...
    eval_data = eval_data.reshape((len(eval_labels), max_input_length))
    rq2_data = input_data.Input_data(
...
def get_out_file(smell, out_folder, model):
    ...
    return os.path.join(
        out_folder, model + smell + "_" + str(now.strftime("%d%m%Y_%H%M") + ".csv")
    )
                ...
                for epoch in epochs:
                    for dropout in dropouts:
                        ...
                        try:
                            ...
                            print(
                                "Skipping combination layer: {}, emb_output: {}, lstm_units: {}, epoch: {}".format(
                                    layer, emb_output, lstm_units, epoch
                                )
                            )
                            print(ex)
                        cur_iter += 1
