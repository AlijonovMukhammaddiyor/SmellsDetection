...
DIM = "1d"
MODE = "train_java"  # "train_cs" or "train_java" representing whether the training data is coming from C# samples or java.
...
def get_out_file(smell):
    ...
    return os.path.join(
        OUT_FOLDER,
        "cnn1d_rq2_" + smell + "_" + str(now.strftime("%d%m%Y_%H%M") + ".csv"),
    )
                    ...
                    for epoch in epochs:
                        ...
                        try:
                            ...
                            (
                                ...
                                input_data, config, smell, OUT_FOLDER, DIM
                            )
                            end_time = time.time()
                            ...
                            print(
                                "Skipping combination layer: {}, filter: {}, kernel: {}, pooling_window: {}".format(
                                    layer, filter, kernel, pooling_window
                                )
                            )
                            print(error)
                            write_result(
...
def run_cnn_with_best_params(input_data, layer, filter, kernel, pooling_window, epoch):
    ...
    try:
        ...
        (
            ...
            average_precision,
            stopped_epoch,
        ) = rq1_cnn_1d.cnn(input_data, config, smell, OUT_FOLDER, DIM, is_final=True)
        end_time = time.time()
        elapsed_time = end_time - start_time
...
if __name__ == "__main__":
    smell = "ComplexMethod"
    training_data_path1 = os.path.join(
    ...
    run_cnn_with_best_params(
        input_data=input_data1, layer=2, filter=16, kernel=5, pooling_window=3, epoch=17
    )

    smell = "ComplexConditional"
    training_data_path2 = os.path.join(
        os.path.join(TRAINING_TOKENIZER_OUT_PATH, smell), DIM
    )
    ...
    run_cnn_with_best_params(
        input_data=input_data2, layer=2, filter=32, kernel=7, pooling_window=2, epoch=9
    )

    smell = "FeatureEnvy"
    training_data_path3 = os.path.join(
        os.path.join(TRAINING_TOKENIZER_OUT_PATH, smell), DIM
    )
    ...
    training_data_path4 = os.path.join(
        os.path.join(TRAINING_TOKENIZER_OUT_PATH, smell), DIM
    )
    ...
    run_cnn_with_best_params(
        input_data=input_data4, layer=1, filter=64, kernel=11, pooling_window=5, epoch=5
    )
