...
TOKENIZER_OUT_PATH = os.path.join(
...
def cnn(data, config, smell, out_folder=OUT_FOLDER, dim=DIM, is_final=False):
    ...
    for i in range(2, config.layers + 1):
        model.add(
            tf.keras.layers.Conv2D(2 * config.filters, config.kernel, activation="relu")
        )
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.MaxPooling2D(config.pooling_window, strides=2))
...
def get_out_file(smell):
    ...
    return os.path.join(
        OUT_FOLDER,
        "cnn2d_rq1_" + smell + "_" + str(now.strftime("%d%m%Y_%H%M") + ".csv"),
    )
...
def run_cnn_with_best_params(
    smell, input_data, conv_layers, filter, kernel, pooling_window, epochs
):
    ...
    try:
        config = configuration.CNN_config(
            conv_layers, filter, kernel, pooling_window, epochs
        )
        start_time = time.time()
        auc, accuracy, precision, recall, f1, average_precision, stopped_epoch = cnn(
            input_data, config, smell, is_final=True
        )
        end_time = time.time()
        elapsed_time = end_time - start_time
        write_result(
        ...
        print(
            "Skipping combination layer: {}, filter: {}, kernel: {}, pooling_window: {}".format(
                conv_layers, filter, kernel, pooling_window
            )
        )
        print(error)
        write_result(
